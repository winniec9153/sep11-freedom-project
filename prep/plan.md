# Plan

## Tool: p5js
## Product: Budgeting game/app

---

## Timeline

## Fonts:
- [Lora](https://fonts.google.com/selection/embed)

### Layout:
 <img width="200" height="450" alt="image" src="https://github.com/user-attachments/assets/fd5083ba-ceb7-400a-8ace-5d701f35646b" />



#### MVP
Plan:
- [x] Finish my Plan.md (deadline: 2/13)
- [x] Use canva to design a visual layout/frame of the budgeting game interface (deadline: 2/20)
  - Example: titlescreen, balance display log, budget trackers and buttons
- [x] Create HTML structure (deadline: 2/25)
- [x] create an area where you insert the amount of money you have and make sure the game will save that number and becomes the player's current balance(deadline: 3/1)
- [x] Make sure that all expense will subtract from the current balance(deadline: 3/5)
- [x] Create a way where income or allowance will add to the balance.(deadline: 3/09)
- [x] Add a category budgeting section (deadline: 3/14)
  - Example: Food, Gas, Entertainment, and others
- [x] Create a section that will display a clear daily spending log (deadline: 3/18)




#### Beyond MVP
- [x] Alert popup when balance is below $25
- [x] Add CSS so there's color and background
<<<<<<< HEAD
- [x ] Hoping to make it responsive on all devices
=======
- [ ] Hoping to make it responsive on all devices
>>>>>>> 00369ef0a619dee4cb71aba845d17c0a3b0e03b9
- [ ] Create a little basic game in side my budgeting game if I have time
- [ ] Make a way to edit transaction incase they made a mistake
- [ ] Create a secton that will display a full monthly log of spending
- [ ] add a money image sprite


<!-- EXAMPLE

## Tool: APIs
## Product: Green Glass Door riddle app

## Timeline

### MVP

- [ ] Front-end
  - [x] Webpage to collect input from user (deadline: 4/15)
  - [ ] Webpage to display "yes, but a ___ can't" or "no, but a ___ can" (deadline: 5/1)
- [x] Back-end
  - [x] Use regex to test whether or not the word can go through the GGD (deadline: 3/1)
  - [x] Use the Twinword API to find related words (deadline: 3/15)
    - [ ] Iterate through the words until an opposite example can be found (deadline: 4/1)

#### Beyond MVP

- [ ] Use another API to make sure the opposite example is a noun
- [ ] Automate notification of API limit to make sure I don’t exceed free quota
- [ ] A multiple choice quizzer that will test the user’s knowledge of the solution

-->





<!-- DO NOT USE THIS YET

#### Peer Feedback

| Name | Glows | Grows |
| -------- | ------- | ------- |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |
|  |  |  |

-->
